  ...
  app.post('/api/setup', async (req, res) => {
    try {
      ...
      if (companyName.length > 200) {
        return res.status(400).json({ message: 'Company name must be under 200 characters' });
      }
      ...
      if (adminName.length > 200) {
        return res.status(400).json({ message: 'Admin name must be under 200 characters' });
      }
  ...
  app.post('/api/collaborators', async (req, res) => {
    try {
      ...
      if (nombre.length > 200) {
        return res.status(400).json({ message: 'Name must be under 200 characters' });
      }
      if (!rol) {
